    ...
    <div className="grid lg:grid-cols-3 gap-6">
            ...
            {daysInMonth.map((day) => {
              ...
              return (
                <button
                  ...
                  className={`sm:h-14 rounded-md flex flex-col items-center justify-center text-sm transition
                                        ${isSelected ? "bg-blue-500 text-white" : isToday ? "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300" : "bg-zinc-50 text-zinc-900 dark:bg-zinc-800/40 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700"}
                                        ${hasOverdue ? "border border-red-400 dark:border-red-500" : ""}`}
                >
            ...
            <div className="space-y-3">
              {getTasksForDate(selectedDate).map((task) => {
                ...
                return (
                  <div
                    key={taskId}
                    className={`bg-zinc-50 dark:bg-zinc-800/40 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition p-4 rounded border-l-4 ${priorityBorders[task.priority] || priorityBorders.LOW}`}
                  >
                    <div className="flex justify-between mb-2">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded text-xs ${typeColors[task.type] || typeColors.TASK}`}
                      >
                        {task.type}
                      </span>
                    </div>
      ...
      <div className="space-y-6">
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start text-sm">
                      <span className="text-zinc-900 dark:text-white truncate mr-2">
                        {task.title}
                      </span>
                      <span
                        className={`text-xs px-2 py-0.5 rounded flex-shrink-0 ${typeColors[task.type] || typeColors.TASK}`}
                      >
                        {task.type}
                      </span>
                    </div>
        ...
        {overdueTasks.length > 0 && (
          <div className="bg-white dark:bg-zinc-900 border border-red-300 dark:border-red-500 border-l-4 rounded-lg p-4">
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between text-sm text-zinc-900 dark:text-white">
                      ...
                      <span className="text-xs px-2 py-0.5 rounded bg-red-200 dark:bg-red-500 text-red-900 flex-shrink-0">
                        {task.type}
                      </span>
                    </div>
