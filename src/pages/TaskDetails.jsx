...
const TaskDetails = () => {
  ...
  const fetchTaskDetails = async () => {
    ...
    // Fetch tasks and find the right one
    try {
      const data = await taskAPI.getByProject(workspaceId, projectId);
      const tsk = data.tasks?.find((t) => (t._id || t.id) === taskId);
      if (tsk) setTask(tsk);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleAddComment = async () => {
    ...
    const comment = {
      id: Date.now(),
      user: { id: user._id, name: user.name, image: user.avatar },
      content: newComment,
      createdAt: new Date(),
    };
        ...
        <div className="p-5 rounded-md border border-gray-300 dark:border-zinc-800 flex flex-col lg:h-[80vh]">
            ...
            {comments.length > 0 ? (
              <div className="flex flex-col gap-4 mb-6 mr-2">
                {comments.map((comment) => (
                  <div
                    key={comment.id}
                    className={`sm:max-w-4/5 dark:bg-gradient-to-br dark:from-zinc-800 dark:to-zinc-900 border border-gray-300 dark:border-zinc-700 p-3 rounded-md ${comment.user.id === user?._id ? "ml-auto" : "mr-auto"}`}
                  >
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-end gap-3">
            ...
            <button
              ...
              className="bg-gradient-to-l from-blue-500 to-blue-600 text-white text-sm px-5 py-2 rounded"
            >
              Post
            </button>
          </div>
